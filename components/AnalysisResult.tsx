...
const Card: React.FC<{
  ...
  <div className="bg-white rounded-2xl shadow-sm border border-warm-100 overflow-hidden mb-6 transition-transform hover:scale-[1.01] duration-300">
    <div className={`px-4 py-4 border-b border-gray-50 flex items-center gap-2 ${colorClass} bg-opacity-10`}>
      {icon}
      <h3 className="font-cute text-xl tracking-wide">{title}</h3>
    </div>
    <div className="p-5">
      {children}
    </div>
...
// Helper to format the overall comment with bold subtitles
const FormattedComment: React.FC<{ text: string }> = ({ text }) => {
  ...
  return (
    <div className="space-y-6 font-sans">
      {lines.map((line, idx) => {
        // Regex to extract title in brackets 【Title】
        // Captures: 1=Title text, 2=Middle stuff (emojis), 3=Content after colon
        const match = line.match(/^.*?【(.*?)】(.*?)[：:](.*)$/);
        
        // Fallback match if colon is missing
        const simpleMatch = !match ? line.match(/^.*?【(.*?)】(.*)$/) : null;

        if (match || simpleMatch) {
          const m = match || simpleMatch!;
          const title = m[1]; // e.g. 温暖抱抱
          const decoration = m[2] || ''; // e.g. 🌟
          const content = m[match ? 3 : 2];

          // Determine specific styling or icon based on title keywords if explicit icons aren't in text
          let iconChar = '';
          if (title.includes('温暖抱抱')) iconChar = '🌟';
          else if (title.includes('成长小贴士')) iconChar = '🚀';
          else if (title.includes('未来寄语')) iconChar = '🌈';
          
          // Use parsed decoration if available, otherwise fallback to default icon
...
const AnalysisResult: React.FC<AnalysisResultProps> = ({ analysis, imagePreviews, onReset }) => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4 pb-24">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="w-full lg:w-5/12 space-y-6">
           ...
           <div className="bg-white rounded-2xl shadow-sm p-4 border border-warm-100">
              ...
              {imagePreviews.length > 1 && (
                <div className="flex gap-2 mt-3 overflow-x-auto pb-2 px-1 scrollbar-hide">
           ...
           {/* OCR Result - Specifically formatted */}
           <div className="bg-white rounded-2xl shadow-sm p-4 border border-warm-100">
             <h3 className="font-bold text-warm-800 mb-3 flex items-center">
               <MessageCircle size={18} className="mr-2" />
               识别内容
             </h3>
             <div className="bg-orange-50/30 p-5 rounded-xl text-gray-700 leading-loose font-sans min-h-[200px] max-h-[60vh] overflow-y-auto border border-orange-100 text-base">
                {analysis.title && (
                  <div className="mb-4">
        ...
        <div className="w-full lg:w-7/12">
                ...
                {analysis.corrections.map((item, idx) => (
                  <div key={idx} className="relative bg-white border border-purple-100 rounded-xl p-5 shadow-sm hover:shadow-md transition-all duration-300 group overflow-hidden">
                    ...
                    <div className="mb-3 pl-3 border-l-4 border-gray-200">
                      ...
                      <p className="text-gray-500 text-sm decoration-gray-300">{item.original}</p>
                    </div>
          ...
          >
            <div className="relative bg-warm-50 p-6 rounded-2xl border border-warm-200/60 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)]">
              ...
              <div className="mt-6 flex items-center justify-end gap-3 opacity-80">
                ...
                <span className="text-warm-800 font-cute text-lg transform -rotate-2 inline-block">你的AI作文魔法师</span>
              </div>
            </div>
          </Card>
      ...
      {/* Floating Action Button for Mobile/Desktop consistency */}
