...
const FileUpload: React.FC<FileUploadProps> = ({ onFileSelect }) => {
  ...
  // Initialize counter from local storage or default to 1888
  useEffect(() => {
  ...
  const processFiles = (files: FileList | File[]) => {
    ...
    // Increment usage counter on valid upload
    incrementUsage();
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-4">
      ...
      <div className="mb-6 text-center">
        <label className="block text-warm-700 font-medium mb-4 text-lg font-cute">
          请选择年级，AI老师会因材施教哦
        </label>
      ...
      <div
        className={`
          ...
          border-3 border-dashed rounded-3xl p-10 transition-all duration-300
      ...
      >
        ...
        <input
          ...
          className="hidden"
          accept="image/*"
          capture="environment" // This triggers the camera on mobile directly
          onChange={(e) => e.target.files && processFiles(e.target.files)}
        />
        ...
        <h3 className="text-2xl font-cute text-warm-800 mb-3">
          点击或拖拽上传作文图片
        </h3>
